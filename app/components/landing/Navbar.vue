<template>
  ...
  >
    <div class="mx-auto flex h-12 max-w-7xl items-center justify-between px-6 relative">
      ...
      <nav class="hidden md:flex items-center p-1 bg-muted/50 rounded-full border border-border/20 absolute left-1/2 -translate-x-1/2 backdrop-blur-sm">
      ...
      <div class="flex flex-1 items-center justify-end gap-2 sm:gap-3">
        ...
        <div class="hidden sm:flex items-center p-1 bg-muted/30 rounded-lg border border-border/10">
          <Button variant="ghost" size="icon" class="h-8 w-8 rounded-md hover:bg-background shadow-none transition-transform active:scale-95" @click="toggleColorMode">
            <ClientOnly>
              <HugeiconsIcon :icon="colorMode.value === 'dark' ? Sun01Icon : Moon01Icon" :size="18" />
              <template #fallback>
          ...
          <DropdownMenu>
            <DropdownMenuTrigger as-child>
              <Button variant="ghost" size="icon" class="h-8 w-8 rounded-md hover:bg-background shadow-none transition-transform active:scale-95">
        ...
        <div class="hidden md:flex items-center gap-2">
          <Button variant="ghost" size="sm" class="font-medium hover:bg-primary/5 transition-colors" @click="goToLogin">
            {{ $t('auth.login_title') }}
          </Button>
          <Button size="sm" class="font-semibold shadow-lg shadow-primary/20 hover:shadow-primary/30 hover:-translate-y-0.5 transition-all duration-300" @click="goToLogin">
            {{ $t('auth.register') }}
          </Button>
        </div>
        ...
        <Sheet>
          <SheetTrigger as-child>
            <Button variant="ghost" size="icon" class="md:hidden h-10 w-10 rounded-xl hover:bg-muted transition-colors">
          ...
          <SheetContent side="right" class="w-75 sm:w-100 flex flex-col">
            ...
            <div class="flex-1 py-8 flex flex-col gap-6">
              <div class="flex flex-col gap-2">
                ...
                <SheetClose as-child v-for="item in navItems" :key="item.href">
                  ...
                  >
                    {{ $t(item.label) }}
                    <HugeiconsIcon :icon="ArrowRight01Icon" :size="18" class="opacity-0 -translate-x-2 transition-all group-hover:opacity-100 group-hover:translate-x-0" />
                  </button>
                </SheetClose>
              </div>

              <div class="flex flex-col gap-2">
                <p class="text-xs font-semibold text-muted-foreground uppercase tracking-wider px-2 mb-2">Settings</p>
                ...
                <div class="flex flex-col gap-1 px-2 mt-2">
                   ...
                   <div class="flex gap-2">
                     <Button
                       ...
                       :class="{ 'border-primary bg-primary/5 text-primary': currentLocale === locale.code }"
                       @click="setLocale(locale.code)"
                     >
                       {{ locale.name }}
                     </Button>
            ...
            <div class="pt-6 border-t flex flex-col gap-3">
              <Button class="w-full h-12 rounded-xl text-base font-bold shadow-lg shadow-primary/20" @click="goToLogin">
                {{ $t('auth.register') }}
              </Button>
              <Button variant="ghost" class="w-full h-12 rounded-xl text-base font-medium" @click="goToLogin">
                {{ $t('auth.login_title') }}
              </Button>
            </div>
          </SheetContent>

<template>
  <div class="mx-auto w-full max-w-2xl space-y-8 overflow-hidden px-4 pb-24 md:px-0 md:pb-8 lg:space-y-10 min-h-dvh">
    <!-- LOADING STATE -->
    <div v-if="loading" class="space-y-8">
      <div class="flex flex-col items-center gap-4 rounded-3xl border border-border/50 bg-card/10 p-6 backdrop-blur-sm md:flex-row md:items-start md:gap-6 md:p-8">
    ...
    <div v-else class="animate-in fade-in slide-in-from-bottom-4 duration-500 space-y-8 lg:space-y-10">
      <!-- PROFILE CARD -->
      <section class="relative overflow-hidden rounded-[2.5rem] border border-border/50 bg-card/20 p-6 backdrop-blur-md md:p-8">
        ...
        <div class="relative flex flex-col items-center gap-5 md:flex-row md:items-center md:gap-8">
          ...
          <div class="min-w-0 flex-1 text-center md:text-left">
            ...
            <div class="mt-4 flex flex-wrap justify-center gap-2 md:justify-start">
              ...
              <span v-if="user?.app_metadata?.provider" class="inline-flex items-center rounded-full bg-muted px-2.5 py-0.5 text-xs font-medium text-muted-foreground">
      ...
      <section>
        ...
        <div class="overflow-hidden rounded-3xl border border-border/50 bg-card/20 backdrop-blur-sm transition-all duration-300 hover:bg-card/25">
      ...
      <section>
        ...
        <!-- LOADING: PASANGAN -->
        <div v-if="partnerLoading" class="flex w-full items-center justify-center rounded-3xl border border-border/50 bg-card/10 py-10 backdrop-blur-sm">
        ...
        <div v-else-if="!isPartnered" class="space-y-4">
          <!-- Kirim Undangan -->
          <div class="rounded-3xl border border-border/50 bg-card/20 p-5 backdrop-blur-sm transition-all duration-300 hover:bg-card/25 md:p-6">
            ...
            <div class="space-y-3">
              ...
              >
                <div class="flex min-w-0 items-center gap-3">
                  <div class="flex size-10 shrink-0 items-center justify-center rounded-xl bg-muted/50">
                    <HugeiconsIcon :icon="Mail01Icon" :size="18" class="text-muted-foreground" />
                  </div>
                  <div class="min-w-0">
                    <p class="truncate text-sm font-semibold text-foreground">{{ inv.recipient_email }}</p>
                    <p class="text-[10px] font-bold uppercase tracking-wider text-primary">{{ inv.status }}</p>
                  </div>
                </div>
                <button
              ...
              >
                <div class="flex min-w-0 items-center gap-3">
                  <Avatar class="size-10 border border-border/30">
                    <AvatarImage v-if="inv.sender?.avatar_url" :src="inv.sender.avatar_url" :alt="inv.sender.display_name || ''" />
                ...
                <div class="flex gap-2">
                  <Button size="sm" variant="ghost" class="rounded-lg font-bold text-red-500 hover:bg-red-500/10 hover:text-red-600" @click="onRejectInvite(inv)">
                    Tolak
                  </Button>
                  <Button size="sm" class="rounded-lg font-bold shadow-md shadow-primary/10" @click="onAcceptInvite(inv)">
                    Terima
                  </Button>
                </div>
              </div>
        ...
        <div v-else class="space-y-4">
          <div class="relative overflow-hidden rounded-3xl border border-emerald-500/20 bg-emerald-500/[0.03] p-5 backdrop-blur-sm transition-all duration-300 hover:bg-emerald-500/[0.06] md:p-6">
            ...
            <div class="relative flex items-center gap-4">
              <Avatar class="size-14 border-2 border-emerald-500/20 shadow-lg md:size-16">
                <AvatarImage v-if="partner?.avatar_url" :src="partner.avatar_url" :alt="partner.display_name || ''" />
          ...
          >
            <div class="flex items-center gap-4">
              <div class="flex size-11 items-center justify-center rounded-2xl bg-red-500/10 text-red-500 transition-transform duration-300 group-hover:scale-110">
      ...
      <section>
        ...
        <div class="rounded-3xl border border-red-500/20 bg-red-500/[0.02] p-1.5 backdrop-blur-sm transition-all duration-300 hover:bg-red-500/[0.05]">
          <button
          ...
          >
            <div class="flex items-center gap-4">
              ...
              <div class="min-w-0 text-left">
                <p class="text-sm font-bold text-red-500 md:text-base">{{ $t('settings.logout') }}</p>
                <p class="text-xs font-medium text-red-500/60 md:text-sm">{{ $t('settings.logout_desc') }}</p>
              </div>
            </div>
            <HugeiconsIcon :icon="ArrowRight01Icon" :size="18" class="shrink-0 text-red-500/30 transition-transform duration-300 group-hover:translate-x-1 group-hover:text-red-500/60" />
          </button>

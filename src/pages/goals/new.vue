  ...
  <div class="mx-auto max-w-2xl space-y-8 pb-12 pt-4">
    <div>
      ...
      <h1 class="text-3xl font-black tracking-tighter text-foreground">
        {{ $t('goal_form.title_new') }}
      </h1>
      <p class="mt-1 font-medium text-muted-foreground">{{ $t('goal_form.subtitle') }}</p>
    </div>
      ...
      <div class="grid gap-6 sm:grid-cols-2">
        <div class="space-y-4">
          <div class="space-y-2">
            ...
            <Input id="name" v-model="form.name" :placeholder="$t('goal_form.name_placeholder')" required />
          </div>
        ...
        <div class="space-y-2">
          ...
          <div class="relative">
            <input ref="fileInputRef" type="file" accept="image/png,image/jpeg,image/webp" class="hidden" @change="onFileSelect" />
            ...
            >
              ...
              <div>
                ...
                <p class="mt-0.5 text-xs font-medium text-muted-foreground">PNG, JPEG, WebP — max 5MB</p>
              </div>
            </button>
            ...
            <div v-else class="relative overflow-hidden rounded-2xl border border-border/50">
              ...
              <div class="absolute inset-0 bg-gradient-to-t from-black/40 via-transparent to-transparent opacity-0 transition-opacity hover:opacity-100" />
              <Button
                type="button"
                variant="secondary"
                size="sm"
              ...
              <div class="absolute bottom-3 left-3">
                <span class="rounded-full bg-black/50 px-3 py-1 text-[10px] font-bold text-white backdrop-blur-sm">
                  {{ $t('goal_form.image') }}
                </span>
              </div>
            </div>
...
const { addGoal, uploadGoalImage } = useGoals();
...
const calendarDate = computed({
  ...
  set: (val) => { if (val) form.deadline = val.toString(); },
});
